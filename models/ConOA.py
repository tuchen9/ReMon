...
class ConOA(nn.Module):
    ...
    def data_augmentation(self, features, ratio):
        if self.aug_type == 'Noise':
            ...
            features += torch.sign(features) * F.normalize(random_noise, dim=-1) * ratio
        elif self.aug_type == 'Dropout':
            features = F.dropout(features, ratio)
        return features
    ...
    def cal_loss(self, anchors_embedding, anchors_embedding_m, assets_embedding_m, batch_org_idx):
        ...
        idx_all = torch.cat([batch_org_idx.permute(1,0), batch_org_idx.permute(1,0), queue_org_idx], dim=1)
